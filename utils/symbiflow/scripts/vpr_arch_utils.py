...
def append_metadata(xml_item, meta_name, meta_data):
    ...
    # Check if there is existing meta tag that can be merged
    for item in xml_metadata.findall("meta"):
        if item.attrib["name"] == xml_meta.attrib["name"]:
            if meta_name in ["fasm_features", "fasm_params"]:
                item.text = item.text + "\n" + meta_data
                break
            else:
                assert False, (item.attrib["name"], xml_meta.attrib["name"])           
